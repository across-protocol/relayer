...
`willSucceed()` uses the `eth_estimateGas` result as-is unless the transaction declares a `gasLimitMultiplier`. `Multicall3.tryAggregate(requireSuccess=false, ...)` must declare one: the estimator returns the lowest limit at which the *outer* transaction succeeds, and a batch whose inner calls all ran out of gas still succeeds, because `tryAggregate` catches their reverts. The estimate is a floor rather than a requirement, so submitting it raw can mine a no-op batch with `status: 1` and no events. Covering the batch's total cost isn't sufficient either — the shortfall is the EIP-150 reserve withheld from the largest call, at most 1/64 of the batch (pinned in `test/MultiCallerClient.TryAggregateGas.test.ts`).
