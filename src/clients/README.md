...
EVM and SVM subscribe over websockets and apply quorum in the application layer: `EventManager` tallies each event across `quorum` providers before posting it. Events are bucketed on on-chain identity only (event name, `blockHash`, `transactionHash`, `logIndex`) so that providers disagreeing on `blockNumber`/`transactionIndex` — as HyperEVM RPCs do, depending on whether they count system transactions — still converge on one bucket. A provider is only credited towards quorum if its decoded event arguments also match those of the first provider to report the event; a mismatch is rejected and logged as `across-error`, since it means an RPC is lying about or corrupting event content. The event posted on reaching quorum is always the first-seen one, never the copy supplied by whichever provider happened to complete quorum. TRON's RPCs don't support websockets reliably and expire `eth_newFilter` ids, so the TVM listener instead polls `eth_getLogs` over a single quorum `RetryProvider` — which imposes node quorum on every query (`--quorum` is threaded into `getProvider` as an override) — and reconciles a trailing re-org window:
