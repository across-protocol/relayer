...
In the L1 -> L2 direction, `OFTBridge` reports the path's remaining capacity through the `getMaxL1ToL2TransferAmount` bridge hook, which the rebalance loop uses to clamp the requested amount so an over-capacity deficit is chunked across runs instead of reverting the send. Capacity is read from `quoteOFT`'s `oftLimit.maxAmountLD`, the only field that reports it on every path type: Stargate pools also cap `oftReceipt.amountSentLD`, but legacy-mesh adapters echo the requested amount there, while unmetered paths report a `type(uint64).max` placeholder for `maxAmountLD` to mean "no limit". That placeholder is matched exactly, in both of the encodings mainnet returns — verbatim (the USDT messenger, whose local and shared decimals are both 6) and scaled by the shared -> local decimal conversion rate (the WETH messenger, 18 local vs 6 shared decimals) — because treating anything at or above the unscaled placeholder as uncapped would misread genuine capacity above `type(uint64).max` local units, only ~18.45 tokens on an 18-decimal path, as unlimited and skip the clamp. A bridge reporting zero capacity is skipped for that run rather than clamped to a zero-amount transfer.

Capacity is read once per rebalance and is not reserved across rebalances planned in the same run, so when one run plans two transfers over the same metered path — an ordinary and a shortfall rebalance for one route, or two L2 tokens mapping to one L1 token on a chain — each clamps against the same pre-transfer figure and the second send can still exceed capacity and revert. Reserving would require distinguishing the hook's two meanings, since a per-transfer ceiling (Binance) must not be decremented the way a shared pool (OFT) must; the amounts still clamp correctly in the common single-rebalance-per-path case, and an over-capacity send fails to construct exactly as it did before this hook existed.
