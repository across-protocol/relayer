...
`willSucceed()` sizes a transaction with `eth_estimateGas` and uses the result as-is unless the transaction declares a `gasLimitMultiplier`; one that already carries a `gasLimit` is taken to have been simulated in advance and passes through. `Multicall3.tryAggregate(requireSuccess=false, ...)` must not be sized by estimating itself: the estimator returns the lowest limit at which the *outer* transaction succeeds, and `tryAggregate` catches inner reverts, so a batch whose calls all ran out of gas still succeeds. That estimate describes the cost of *failing*, and submitting it raw mines a batch that did nothing, with `status: 1` and no events — it discarded a 76,064.59 USDC CCTP v2 mint on 2026-08-05 and stalled two OP-stack withdrawals across ~10 runs the same day. Padding it doesn't generalise: the shortfall is bounded by the EIP-150 reserve only while a call's requirement follows what it consumes, and OP-stack `SafeCall.callWithMinGas` gates on `gasleft() >= minGas * 64/63` with `minGas` declared by the withdrawal on L2, ~30x what the reverting path spends.

`buildFinalizationBatches()` in `src/finalizer/index.ts` therefore estimates each call on its own, packs them into batches under `MULTICALL3_BATCH_GAS_CEILING` (EIP-7825 caps a transaction at 2^24 = 16,777,216 gas, far below mainnet's ~60M block limit, and nothing else divides a batch), and sizes each batch as `aggregate()`, where a revert is fatal so the estimator can't undershoot. A batch that won't estimate — some call reverts only in combination — falls back to its summed per-call gas. Packing on per-call estimates leaves batches smaller than necessary, since each estimate carries intrinsic and cold-access gas that `aggregate()` pays once for the batch; it understates only for a call that doesn't estimate at all, which reverts and so spends little. Each batch is enqueued `nonMulticall`, without which the bundler puts them back together — into the transaction that didn't fit, or into a `multicall(bytes[])` Multicall3 doesn't expose, which throws on encode and abandons the chain's whole batch.

Splitting also changes what a chain's transaction hashes prove. `TransactionClient.submit()` submits sequentially and returns the hashes collected before the first failure, so a chain that enqueued several batches can land only some. The hashes don't identify the batches behind them, so `submissionStatus()` reports *every* message on a partially-submitted chain as unconfirmed rather than crediting it to a transaction that may not have carried it: a finalization wrongly logged as complete is one nobody goes looking for. `test/MultiCallerClient.TryAggregateGas.test.ts` and `test/Finalizer.BatchBuilding.test.ts` cover all of this against real Multicall3 bytecode.
