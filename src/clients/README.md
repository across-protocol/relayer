...
Robinhood (chain 4663) WETH withdraws to mainnet over the Arbitrum Orbit canonical bridge (`ArbitrumOrbitBridge` in `CUSTOM_L2_BRIDGE`); Robinhood USDC/USDG continues to use `PaxosTransitL2Bridge`, and the inbound mainnet → Robinhood WETH leg is unchanged. This is the slowest route the inventory currently plans: the rollup's challenge period is 45818 L1 blocks (roughly 6.4 days), recorded as `confirmPeriodBlocks` in `ARB_ORBIT_NETWORK_CONFIGS` (`src/utils/ArbSdkUtils.ts`). That figure is the single source of truth for the route's timing — `getArbitrumOrbitFinalizationTime` converts it to seconds for event lookbacks, `arbStackFinalizer` registers it with `@arbitrum/sdk` verbatim, and Jussi prices the Robinhood → mainnet edge with it rather than the 20-minute canonical default so the multi-day capital lock is not underpriced. Keep it an exact integer: the SDK feeds it to `BigNumber.from()`, which throws `NUMERIC_FAULT` on a fractional value, and `getMessageOutboxStatusAndProof` swallows that error so every affected withdrawal would silently go unfinalized.

Operationally, a withdrawal is in flight for the whole challenge period before the finalizer can execute its outbox message, so operators should expect Robinhood WETH to sit in the pending bucket for roughly a week. During that window `ArbitrumOrbitBridge.getL2PendingWithdrawalAmount` keeps the in-flight amount in the virtual mainnet balance by pairing L2 `WithdrawalInitiated` events against L1 `WithdrawalFinalized` events over a lookback of the challenge period plus one hour. Both gateways can be shared across tokens (`DEFAULT_ARBITRUM_GATEWAY`), so each side is filtered on the non-indexed `l1Token`, which the event decodes as a checksummed string and must therefore be compared case-insensitively rather than against the `EvmAddress` object.
