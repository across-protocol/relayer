...
Recovery-sensitive callers may set `AugmentedTransaction.onBroadcast`. It runs after the transaction is signed locally but _before_ the raw send is dispatched (`provider.sendTransaction` on EVM, `sendRawTransaction` on TVM), supplying the locally-computed hash — so a crash mid-send cannot lose the hash mapping, but a persisted hash is not by itself evidence the node ever received the transaction. It runs again with the latest hash when a repriced replacement becomes canonical, and with the original hash when a rejected same-nonce replacement leaves the original submission live. `submitTransaction()` and `dispatchTransaction()` classify simulation failures as `DefinitiveTransactionFailure`. With `onBroadcast` present, the transaction client also propagates definitive pre-broadcast rejections and mined reverts; a failure of the initial pre-dispatch callback is definitive (nothing was dispatched). Exhausted confirmation propagates as `TransactionConfirmationPendingError` with the submitted hash. A post-dispatch callback failure, or a broadcast transport error whose acceptance is unknown, propagates as `TransactionSubmissionPendingError`; its hash is present when known. Both pending errors preserve a known accepted nonce in the client so a later submission cannot replace the recovery-sensitive transaction accidentally, while a definitive pre-broadcast failure leaves the previously-cached nonce intact (nothing was consumed) except after a nonce-expired rejection, which drops it to force a resync. Callers should roll back only definitive failures and retain recovery state for both pending error types and other ambiguous outcomes. Transactions without `onBroadcast` preserve the legacy partial-response behavior after simulation succeeds.
