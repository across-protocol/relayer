...
Nonce selection queries both the signer's confirmed (`latest`) and `pending` transaction counts. A modest pending backlog is the normal signature of concurrent submitters sharing the signer (e.g. multiple bot instances), so new transactions append behind it at the pending nonce. When the backlog reaches `NONCE_BACKLOG_REPLACE_THRESHOLD` (per-chain override `NONCE_BACKLOG_REPLACE_THRESHOLD_<chainId>`, default 4; invalid overrides are sanitized to the default), the head of the queue is assumed stuck and the new transaction targets the confirmed nonce for replacement, with gas scaling engaging on retry. Because a successful replacement leaves the backlog depth unchanged, each queue head is targeted at most once per confirmation window (subsequent submissions append behind the queue rather than evicting the replacement); replacement re-arms when the head advances, or when the replacement itself fails to confirm within the window *and* the chain has produced blocks that could have tested it — wall-clock expiry alone does not re-arm on a stalled chain. The window and block gates restart when the replacement is actually broadcast (selection-time gates could age out during gas pricing and signing): the reservation is recorded synchronously at broadcast, with the broadcast-time block baseline patched out-of-band so a slow block read delays neither the protection nor the response. The stalled-wait confirmation rebroadcast holds the same reservation. Reservations are held per nonce, so protection for the head coexists with reservations elsewhere in the queue, and entries for consumed nonces are pruned on every selection as the confirmed count advances (broadcasts on pending-unsupported providers record reservations too, so pruning cannot be confined to the deep-backlog path). A replacement that definitively fails before reaching the provider (gas quoting, simulation, local signing errors, or rejection with retries exhausted) releases its reservation immediately; release requires ownership, and any broadcast refresh invalidates outstanding release tokens, so an aged-out attempt failing late cannot strip protection from a live transaction. The unsupported-`pending` classification is tracked per provider (one chain can mix providers, e.g. spray vs. standard), applies only to providers that have never served a pending count, and a first-observation failure is retried once before the tag is classified — once support has been observed (recorded the moment a pending read succeeds, independent of the paired `latest` read), a pending-count failure is treated as transient and propagated for retry. Providers genuinely without `pending` blockTag support submit at the confirmed nonce in replacement mode, since appending is not an option and an occupied nonce can only be fee-escalated; a warm cache that leads the confirmed count remains authoritative there, but likewise retains replacement mode so a collision escalates in place at the cached nonce. A `REPLACEMENT_UNDERPRICED` rejection is handled according to intent: replacement-mode submissions scale gas and retry at the same nonce, while append-mode submissions treat it as a lost race against a concurrent submitter and re-sync to the next free nonce instead of bidding against an in-flight transaction. A rejection proves the nonce is occupied, so if the re-selection fails to advance past it (the provider's pending count is stale), the retry escalates at that nonce — advancing the fee ladder immediately, since the rejection proved the incumbent is priced at/near market — instead of repeating an unscaled submission; if the occupant is a reservation-protected replacement from this process, append mode is held instead of escalating into it (protection applies the same block-progress criterion as re-arming, so a stalled chain does not end it on wall-clock expiry alone). Cached per-signer nonces in `submit()` are reconciled through nonce selection on the first warm-cache submission per signer per batch: an append-mode selection takes effect only when it leads the cache (pending counts can lag this client's own submissions), while a deep-backlog replacement selection is adopted as-is so that a long-lived client still replaces a stuck head rather than appending behind it indefinitely. Replacing a head resets the cache to it while the rest of the backlog remains in flight, so subsequent transactions in the batch are floored at the observed backlog tail or the cache's high-water mark, whichever is greater (a lagging pending count must not pull the floor back onto this client's own in-flight nonces), rather than incrementing from the head; a cold-cache first submission leaves the signer unmarked, and a response nonce landing behind the requested nonce (a collision retry that replaced the head inside `_runTransaction`) re-opens reconciliation and floors the remainder of the batch at the requested nonce, so the tail stays protected even when the follow-up reconciliation itself fails transiently. Reconciliation marks a signer done only on success, so a transient failure is re-attempted by the next submission. The per-signer nonce cache is a monotonic high-water mark: a lower response nonce — a head replacement, or a late response from an interleaved concurrent `submit()` — never moves it backward onto occupied territory.
