...
Binance routes carry a second, independent constraint: whether Binance is currently accepting transfers on them. Binance enables and disables deposits and withdrawals per coin and network — during chain upgrades, wallet maintenance, or unilaterally — and keeps the suspended pair listed in its API, so `hasBinanceRoute()` (a static check over the configured L2 bridges) cannot see it. Each `InventoryClient` update snapshots Binance's `accountCoins` response via `BinanceClient.refresh()` and logs the live state of the routes in use: a warning while any is suspended, and an info log when one reopens. `BinanceClient.canDrainToHubChain()` gates on that snapshot, and both `isQuicklyRebalanced()` and `canWithdraw()` consult it, so a suspended route stops counting as a fast drain for repayment-chain selection.

Availability is fail-open: only an explicit `false` from Binance closes a route. A missing snapshot (first cycle, or an API failure), an omitted flag, or an unrecognised coin/network all count as available, so a Binance outage degrades to the previous behaviour of not checking rather than stranding every route at once.
