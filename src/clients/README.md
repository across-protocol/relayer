...
The finalizer therefore sizes a batch from its calls' own estimates — see `buildFinalizationBatches()` in `src/finalizer/index.ts` — and submits `tryAggregate`, so a call that starts reverting after the pre-flight still can't take the rest of the batch with it. Estimating per call also names the call that reverts, which an estimate of the batch cannot. `MULTICALL3_TRY_AGGREGATE_GAS_MULTIPLIER` survives only for a batch whose calls have all stopped estimating, where the `tryAggregate` estimate is the last resort. `test/Finalizer.BatchBuilding.test.ts` and `test/MultiCallerClient.TryAggregateGas.test.ts` pin these properties against real Multicall3 bytecode.
