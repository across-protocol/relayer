...
In the L1 -> L2 direction, `OFTBridge` reports the path's remaining capacity through the `getMaxL1ToL2TransferAmount` bridge hook, which the rebalance loop uses to clamp the requested amount so an over-capacity deficit is chunked across runs instead of reverting the send. Capacity is read from `quoteOFT`'s `oftLimit.maxAmountLD`, the only field that reports it on every path type: Stargate pools also cap `oftReceipt.amountSentLD`, but legacy-mesh adapters echo the requested amount there, and vanilla OFTs report `type(uint64).max` for `maxAmountLD` to mean "no limit". A bridge reporting zero capacity is skipped for that run rather than clamped to a zero-amount transfer.
