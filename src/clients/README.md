...
For transactions submitted with `ensureConfirmation: true`, confirmation is awaited with a bounded wait (6 s, or 24 s on mainnet) that retains ethers' replacement detection. The wait bound is only a sampling cadence — replacement decisions are block-driven: a transaction is resubmitted at the same nonce once the chain has produced at least 2 blocks without including it, with freshly-priced gas scaled up by a compounding per-attempt fee bump (the larger of the chain's priority fee scaler and 1.1, capped by `MAX_GAS_RETRY_SCALER_DEFAULT[_<chainId>]`) applied to both fee caps and floored at 1.1x the outgoing transaction's fee caps (a falling gas estimate must not underprice the replacement), so each replacement can displace its predecessor without relying on the RPC to report `REPLACEMENT_UNDERPRICED` (not guaranteed to surface on every submission route). An externally-replaced transaction (`TRANSACTION_REPLACED`) is resubmitted immediately, except when the mined replacement carries identical calldata ("repriced" — i.e. the original won the race against its own replacement), which is adopted as-is. Reverted transactions propagate as submission failures; exhausted resubmissions emit an error-level log (paging the on-call) and return the unconfirmed response.
