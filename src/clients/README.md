...
Binance (`BinanceCEXBridge`) is accounted differently from the on-chain bridges. There are no withdrawal events to read, so pending L2 -> L1 volume is derived from the Binance REST deposit/withdrawal histories: deposits into the exchange on the L2 side, netted against completed Ethereum withdrawals to the monitored EOA for the same coin (`getOutstandingBinanceDeposits`). The Binance account is shared, so the deposit list is first narrowed to deposits this relayer actually funded: `filterDepositsFromAddress` runs one paginated `Transfer(relayer, depositAddress)` `getLogs` over the search window and keeps the deposits whose `txId` appears in it, which is flat in deposit count. Native-token deposits emit no `Transfer`, so `BinanceCEXNativeBridge` overrides that hook and reads `receipt.from` per deposit instead. Two consequences for the accounting window:

- The lookback is **4 hours** (`pendingWithdrawalLookbackPeriodSeconds`), above the 2-hour `BaseL2BridgeAdapter` default. The exchange-side withdrawal is fast, but it cannot be requested until Binance finishes confirming the L2 deposit, and that dominates the round trip — measured deposit-to-withdrawable times on zkSync Era USDC were 34, 54 and 69 minutes. A 1-hour window aged in-flight deposits out of the pending total before Binance released them, so the relayer briefly stopped counting capital it still owned and under-reported its own inventory. 4 hours keeps them visible with headroom while staying inside the finalizer's own deposit lookback.
- Deposits in a terminal failure state (`REJECTED`, `WRONG_DEPOSIT`) are excluded, matching the statuses the Binance finalizer refuses to finalize. Binance will never credit them, so they can never be withdrawn to L1; counting them would inflate the hub's virtual balance for the whole window and suppress rebalances that are actually needed. Pending, credited and awaiting-confirmation deposits are all still in flight and stay counted.

Because deposits and withdrawals are queried from the same cutoff, a deposit/withdrawal cycle that straddles the start of the window contributes a withdrawal whose originating deposit is out of scope, which under-states pending volume until the window clears it. This is inherent to the netting model — Binance withdrawals carry no metadata identifying the L2 that funded them — and is the same gap called out in the finalizer, which clamps its equivalent subtraction at zero.
