...
Recovery-sensitive callers may set `AugmentedTransaction.onBroadcast`. It runs after the RPC accepts an EVM or TVM transaction and again if a repriced replacement becomes canonical, supplying the latest transaction hash each time. `submitTransaction()` and `dispatchTransaction()` classify simulation failures as `DefinitiveTransactionFailure`. With `onBroadcast` present, the transaction client also propagates definitive pre-broadcast rejections and mined reverts; exhausted confirmation propagates as `TransactionConfirmationPendingError` with the submitted hash. A callback failure, or a TVM broadcast transport error whose acceptance is unknown, propagates as `TransactionSubmissionPendingError`; its hash is present when known. Both pending errors preserve a known accepted nonce in the client so a later submission cannot replace the recovery-sensitive transaction accidentally. Callers should roll back only definitive failures and retain recovery state for both pending error types and other ambiguous outcomes. Transactions without `onBroadcast` preserve the legacy partial-response behavior after simulation succeeds.
