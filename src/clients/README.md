...
This client batches enqueued transactions per chain and submits them through Multicall3, falling back to individual submission for transactions that carry value or set `nonMulticall`. Each chain's batch is simulated as a whole first; if the batch simulation fails, the transactions are re-simulated individually and only the ones that pass are submitted, so a single bad transaction does not block the rest.

A failed **bundle** simulation is logged at error level unless the revert reason is one of the specifically recognised benign ones (`isKnownRevertReason` / `knownRevertReasons` — races against another bot, e.g. `relay filled` or `Already claimed`). Generic reverts are deliberately **not** forgiven for a bundle, even though `canIgnoreRevertReason` forgives them for an `aggregate`/`multicall` method: Multicall3 discards the inner revert reason, so a failing bundle reports only `Multicall3: call failed`, and treating that as benign silently drops every batched transaction for that chain on every run. This narrower rule applies **only** to genuine bundles — a lone queue entry is submitted unwrapped (as are `nonMulticall` transactions), reverts with its own reason, and so keeps the more forgiving per-method suppression that stops ordinary relayer/dataworker races from paging the on-call.

Callers that need one failing call not to abort the batch on-chain should submit via `tryAggregate(requireSuccess=false)` rather than `aggregate()` — the finalizer does this, and pre-simulates each call through the multisender's `tryAggregate` (so that `msg.sender` matches production and the inner revert reason is legible) in order to name and drop just the culprit.
