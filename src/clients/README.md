...
A call that no longer estimates has no size, so it is batched separately rather than charged against a limit summed from its neighbours. `tryAggregate` contains a revert, but not gas exhaustion: an inner call that runs the batch out of gas reverts the outer transaction with it, and since a sized batch carries an explicit `gasLimit` it reaches the chain without a further simulation to catch that. Measured, a call consuming 636k inside a batch sized at 753k for its other calls starved the call after it, and a heavier one reverted the whole transaction. Such a batch has no sized limit and falls back to `MULTICALL3_TRY_AGGREGATE_GAS_MULTIPLIER` on the `tryAggregate` estimate, which is the last resort for a cost nobody could measure.
