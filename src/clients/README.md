...
### `maxTries` — bounding the confirmation wait

`AugmentedTransaction.maxTries` caps how long confirmation may take, defaulting to `DEFAULT_CONFIRMATION_TRIES` (10). It is not a wait count: each timeout that decides to resubmit recurses with one fewer try, so the worst case is `M(M+1)/2` waits. At the default that is 55 — roughly **22 minutes on mainnet** at its 24 s cadence. A caller working to a deadline must lower it: `maxTries: 4` bounds the wait at ≤10 waits, ~240 s. Lowering it trades resubmission attempts, and therefore stuck-transaction replacement, for predictability.

### `onBroadcast` — recording submission intent durably

`AugmentedTransaction.onBroadcast?: (response: TransactionResponse) => Promise<void>` is invoked once a transaction is broadcast and its hash is known, **before** the confirmation wait. It exists for callers that must survive being killed mid-confirmation: a caller that only records success after the receipt has no record of a transaction already on the wire. `src/deposit-address-service` uses it to persist `broadcast_pending`.

- **Rejections are logged and swallowed.** The transaction is already on the wire, so losing the caller's record must not lose the transaction. A caller for whom the record is load-bearing must therefore observe the failure itself — catch inside the hook and check afterwards — rather than expect it to propagate. A failed *broadcast* still propagates normally.
- **Invoked again whenever the hash to track changes.** A repriced replacement is notified with the replacement hash, because only the replacement will ever have a receipt — including when that replacement reverted, since a caller left holding the original hash could never resolve it. The client's own resubmits (timeout, and non-repriced replacement) recurse into `_submit` and so notify their new hash from there.
- **Fires on both submission paths**, dispatcher and non-dispatcher, since `dispatch()` and `submit()` both route through `_submit`.
- **TVM reports an un-prefixed `txid`**, which is what the hook receives verbatim; callers that later look the hash up over eth-JSON-RPC must `0x`-prefix it.
