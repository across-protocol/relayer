...
Batch simulation failures are logged at error level unless the revert reason is one of the specifically recognised benign ones (`knownRevertReasons` — races against another bot, e.g. `relay filled` or `Already claimed`). Generic reverts are deliberately **not** forgiven at the batch level, even for `aggregate`/`multicall` methods where `canIgnoreRevertReason` forgives them for individual transactions: Multicall3 discards the inner revert reason, so a failing bundle reports only `Multicall3: call failed`, and treating that as benign silently drops every batched transaction for that chain on every run. Callers that need one failing call not to abort the batch on-chain should submit via `tryAggregate(requireSuccess=false)` rather than `aggregate()` — the finalizer does this, and pre-simulates each call individually so the culprit can be named.
