...
Withdrawals initiated by the relayer's own address do not emit `TokensBridged`, so the zkSync finalizer additionally discovers them from `BridgeBurn` on the L2 native token vault, `Withdrawal` on the L2BaseToken contract, and `WithdrawalInitiated` on the standalone USDC bridge where the chain has one, filtered by the addresses in `FINALIZER_WITHDRAWAL_TO_ADDRESSES`. Without that address list configured the withdrawals will initiate but never be finalized. The Polygon finalizer does the same for PoS burns, discovering `Transfer(sender -> 0x0)` on each hub-enabled child token — Polygon has no single bridge contract to watch, since the burn is emitted by the token itself. USDC and USDT are excluded because they exit over CCTP and OFT; their burns are otherwise indistinguishable from a PoS burn. Polygon is registered as a chain-level default in `CANONICAL_L2_BRIDGE` (`PolygonERC20Bridge`) so any standard bridged ERC20 exits over the PoS bridge, with those two overriding it in `CUSTOM_L2_BRIDGE`. A PoS exit credits whoever burned the tokens, so EOA withdrawals skip the `PolygonTokenBridger.retrieve()` step that SpokePool withdrawals need, and `constructWithdrawToL1Txns` rejects a recipient other than the signer rather than sending someone else's funds to it.
