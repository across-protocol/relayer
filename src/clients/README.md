...
When `L1_TOKENS_OVERRIDE` is set, `getL1Tokens()` restricts inventory management to the override tokens, even if the inventory config references more. Tokens outside the override are ignored by inventory updates, rebalances, and excess-balance withdrawals, because the `TokenClient` only tracks balances for the override set. This lets a bot reuse a broader inventory config without generating rebalance candidates it can neither fund nor account for locally. An empty override leaves the full config in effect.
...
An L1-to-L2 bridge adapter may implement `prepareL1ToL2Transfer` to validate, cap, or decline an amount and reserve adapter-owned capacity before InventoryClient reserves its virtual balance. InventoryClient accounts for the accepted amount and calls `releaseL1ToL2Transfer` when a prepared candidate is abandoned or definitively rejected before broadcast. Adapters that initiate transfers directly must retain their own post-broadcast lifecycle state: the Binance stablecoin-swap bridge stores Redis orders that the swap rebalancer progresses, and its bridge-event queries stay empty so the same pending transfer is not counted twice.
