...
`willSucceed()` sizes a transaction with `eth_estimateGas` and the gas limit is used as-is unless the transaction declares a `gasLimitMultiplier`. `Multicall3.tryAggregate(requireSuccess=false, ...)` must declare one, because the estimator answers the wrong question: it finds the lowest limit at which the *outer* transaction succeeds, and a batch whose inner calls all ran out of gas still succeeds — `tryAggregate` catches their reverts and reports `success=false` per call. The estimate is therefore a floor rather than a requirement, and submitting it raw mines a batch that did nothing, with `status: 1` and no events (this discarded a 76,064.59 USDC CCTP v2 mint on 2026-08-05). A fixed multiplier is sound because the shortfall is bounded: it is the EIP-150 reserve withheld from the largest call, so at most 1/64 of the batch. Note that covering the batch's total cost is *not* sufficient — in that incident the estimate already exceeded total cost and still starved the inner call, because the shortfall is about how gas is distributed across frames. `test/MultiCallerClient.TryAggregateGas.test.ts` pins both properties against real Multicall3 bytecode.
