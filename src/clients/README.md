...
- On TRON the RPC URL must target QuikNode's eth-JSON-RPC path (`…/jsonrpc`); the bare token URL is the TronGrid API and 404s for `eth_*` calls. Transaction submission needs the opposite endpoint: TronWeb speaks the native Tron HTTP API, resolving paths like `wallet/createtransaction` against its `fullHost`, so `getTronWebFromEvmSigner` strips the `/jsonrpc` suffix to recover the native base. Keep the configured URL on the JSON-RPC path and let that derivation handle submission.
