...
`willSucceed()` sizes a transaction with `eth_estimateGas` and the gas limit is used as-is unless the transaction declares a `gasLimitMultiplier`; a transaction that already carries a `gasLimit` is taken to have been simulated in advance and passes through. `Multicall3.tryAggregate(requireSuccess=false, ...)` must not be sized by estimating itself: the estimator finds the lowest limit at which the *outer* transaction succeeds, and a batch whose inner calls all ran out of gas still succeeds — `tryAggregate` catches their reverts and reports `success=false` per call. The estimate therefore describes the cost of *failing*, and submitting it raw mines a batch that did nothing, with `status: 1` and no events (this discarded a 76,064.59 USDC CCTP v2 mint on 2026-08-05, and stalled two OP-stack withdrawals across ~10 runs the same day). Padding it is not a general fix: the shortfall is only bounded by the EIP-150 reserve while a call's gas requirement is a function of what it consumes. OP-stack `SafeCall.callWithMinGas` breaks that — it gates on `gasleft() >= minGas * 64/63`, where `minGas` was declared by the withdrawal on L2 and is ~30x what the reverting path spends, so the gate is invisible to the estimator at any multiplier. The finalizer instead sizes the batch by estimating the same calls as `aggregate()`, where an inner revert is fatal to the outer call and the estimator cannot return a limit below every call's true requirement, then submits `tryAggregate` so a revert appearing after the pre-flight still can't take the rest of the batch with it. Sizing that way lets one bad call spoil the *estimate*, so the fallback is per-call estimates summed — not the `tryAggregate` estimate, which would let that one call silently re-impose the floor on every other call in the batch. The sum over-provisions when several calls carry a large `minGas`, because the reserve is a floor each call must be *given* rather than gas it spends, so successive calls reuse it instead of accumulating; that is a loose limit rather than a loose spend. `test/MultiCallerClient.TryAggregateGas.test.ts` pins these properties against real Multicall3 bytecode.

A sized batch is then split to fit in one transaction. EIP-7825 caps a single transaction at 2^24 = 16,777,216 gas, two orders of magnitude below mainnet's ~60M block gas limit, so the cap on the transaction rather than the block is what bounds a batch — and nothing else divides it, since the finalizer enqueues one `tryAggregate` per chain and `MultiCallerClient` passes a single-element multisender chunk through untouched. `chunkFinalizationBatch()` sizes the whole batch first, so a batch under `MULTICALL3_BATCH_GAS_CEILING` costs no extra RPC and is submitted exactly as before; only an oversized one pays for per-call estimates and is packed greedily into chunks that each size below the ceiling. Packing against per-call estimates leaves chunks smaller than strictly necessary, which is the safe direction: the sum over-counts a shared `minGas` reserve, and each per-call estimate also carries its own intrinsic and cold-access gas that `aggregate()` pays once for the batch (measured at ~40k a call, far more than the Multicall3 loop overhead the sum omits). It understates only where a call doesn't estimate on its own and so contributes nothing to packing, which the pre-flight permits because it admits calls on `eth_call` rather than `eth_estimateGas`. Each resized chunk is therefore checked against the ceiling, and a chunk whose padded limit still exceeds it is logged — as an error once past `EIP7825_TXN_GAS_CAP`, where submission would actually be rejected, and the ~1.8M between ceiling and cap is the margin absorbing anything smaller. Each chunk is then enqueued `nonMulticall`, which is what makes the split reach the wire: chunks share a target contract, so the bundler would otherwise put them back together — as `multicall(bytes[])` when no multisender is reachable, which Multicall3 doesn't expose so the chain's whole batch dies on encoding, or as one re-aggregated transaction when it is, which is the transaction that didn't fit.

Splitting also changes what a chain's transaction hashes prove. `TransactionClient.submit()` submits sequentially and, on the first failure, logs and returns the hashes it already collected without attempting the rest — so a chain that enqueued several chunks can come back having landed only some. The hashes don't identify the chunks behind them and a chunk's calls aren't retained past enqueueing, so `submissionStatus()` reports *every* message on a partially-submitted chain as unconfirmed rather than crediting it to a transaction that may not have carried it. Over-warning on the messages that did land is the deliberate direction: a finalization wrongly logged as complete is one nobody goes looking for. `test/Finalizer.BatchChunking.test.ts` covers both.
