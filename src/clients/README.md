...
`Multicall3.tryAggregate(requireSuccess=false, ...)` must not be sized by estimating itself. The estimator finds the lowest limit at which the *outer* transaction succeeds, and a batch whose inner calls all ran out of gas still succeeds — `tryAggregate` catches their reverts and reports `success=false` per call. The estimate therefore describes the cost of *failing*, and submitting it raw mines a batch that did nothing, with `status: 1` and no events (this discarded a 76,064.59 USDC CCTP v2 mint on 2026-08-05, and stalled two OP-stack withdrawals across ~10 runs the same day).

Padding that estimate is not a general fix. The shortfall is bounded by the EIP-150 reserve only while a call's gas requirement is a function of what it consumes; OP-stack `SafeCall.callWithMinGas` breaks that, gating on `gasleft() >= minGas * 64/63` where `minGas` was declared by the withdrawal on L2 and is ~30x what the reverting path spends. The gate is invisible to the estimator at any multiplier.

The finalizer therefore sizes a batch from its calls' own estimates — see `buildFinalizationBatches()` in `src/finalizer/index.ts` — and submits `tryAggregate`, so a call that starts reverting after the pre-flight still can't take the rest of the batch with it. Estimating per call also names the call that reverts, which an estimate of the batch cannot.

The summed estimates price the calls but not the `tryAggregate` around them, which pays its own intrinsic gas and calldata, dispatches each call, and loses 63/64 of what remains forwarding to it. A batch of several calls absorbs that incidentally, from the 21,000 intrinsic gas each standalone estimate carries and the batch pays once; a batch of one has no such slack, and a proportional multiplier can't supply what the sum doesn't contain — a lone call estimating at 48,773 needs 54,870 and would be handed 53,650. `MULTICALL3_BATCH_GAS_OVERHEAD` covers the wrapper as a fixed allowance, and `MULTICALL3_BATCH_GAS_MULTIPLIER` is left to cover drift against a state that moved since the estimate.

A call that no longer estimates is dropped from the run, because no limit could honestly carry it. It can't share a sized batch: `tryAggregate` contains a revert but not gas exhaustion, so a call that spends before it fails spends out of a limit summed from its neighbours and takes them down with it — measured, 636k consumed inside a batch sized at 753k for its other calls starved the call after it, and a heavier one reverted the whole transaction. Since a sized batch carries an explicit `gasLimit`, it reaches the chain without a further simulation to catch that. Nor is a batch of its own worth submitting: alone it has no neighbours to protect, its only sizing would be the `tryAggregate` estimate of it failing, and `tryAggregate` would mine that failure as a *successful* transaction — which `submissionStatus()`, counting hashes rather than inner success flags, would report as a finalization that landed. Dropping costs a run of latency, since finalizations are rediscovered from chain state on the next one, and reports the message honestly meanwhile.
