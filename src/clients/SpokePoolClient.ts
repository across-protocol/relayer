...
export class SpokePoolClient {
  ...
  getExecutedRefunds(relayerRefundRoot: string): { [tokenAddress: string]: { [relayer: string]: BigNumber } } {
  ...
  async update(eventsToQuery?: string[]): Promise<void> {
    ...
    if (eventsToQuery.includes("FundsDeposited")) {
      ...
      // Store in-memory any newly fetched events from RPC.
      if (depositEvents.length > 0)
        this.log("debug", `Queried ${depositEvents.length} new deposit events for chain ${this.chainId}`, {
    ...
    if (eventsToQuery.includes("FilledRelay")) {
      ...
      if (fillEvents.length > 0)
        this.log("debug", `Queried ${fillEvents.length} new fill events for chain ${this.chainId}`, {
