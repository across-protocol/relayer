...
### Orbit gas funding during pool-leaf execution

When executing pool-rebalance leaves, the `HubPool` relays the bundle's roots and net token amounts to each `SpokePool`. For Arbitrum/Orbit chains that path goes through the Arbitrum Inbox, which charges an L1→L2 submission fee paid in ETH (or a custom gas token). `_executePoolRebalanceLeaves` therefore pre-funds the `HubPool` for those leaves via `loadEthForL2Calls`, sizing the amount from `ARBITRUM_ORBIT_L1L2_MESSAGE_FEE_DATA`.

Universal chains (`UNIVERSAL_CHAINS`, e.g. Robinhood) are technically Orbit chains (`chainIsOrbit` returns `true`) but relay HubPool→SpokePool roots **and** tokens via the Succinct/Helios SP1 light-client bridge (`HubPoolStore`), not the Orbit Inbox. The `HubPool` needs no pre-loaded ETH for those messages and these chains have no `ARBITRUM_ORBIT_L1L2_MESSAGE_FEE_DATA` entry, so pool-leaf execution skips the gas-funding step for them and queues only `executeRootBundle`. This mirrors the finalizer, which routes `UNIVERSAL_CHAINS` through the Helios L1→L2 finalizer. (Note: this is independent of inventory rebalancing, where the relayer's own wallet pays its own submit value via the `ArbitrumOrbitBridge` adapter.)