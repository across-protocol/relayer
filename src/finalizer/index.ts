  ...
  for (const chainId of configuredChainIds) {
    ...
    if (chainId === 42161) {
      ...
      for (const l2Message of finalizableMessages) {
        await finalizeArbitrum(logger, l2Message.message, l2Message.proofInfo, l2Message.info, hubPoolClient, isDryRun);
      }
    } else if (chainId === 137) {
      ...
      for (const event of canWithdraw) {
        await finalizePolygon(posClient, hubPoolClient, event, logger, isDryRun);
      }
      for (const l2Token of getL2TokensToFinalize(tokensBridged)) {
        await retrieveTokenFromMainnetTokenBridger(logger, l2Token, hubSigner, hubPoolClient, isDryRun);
      }
    } else if (chainId === 10) {
      ...
      for (const message of finalizableMessages) {
        await finalizeOptimismMessage(hubPoolClient, crossChainMessenger, message, logger, isDryRun);
      }
    }
  }
}
