...
export function getL2MessageServiceContractFromL1ClaimingService(
  ...
  const iface = new Interface(original.interface.fragments);
...
export function determineMessageType(
  ...
  if (_calldata === "0x" && _value > 0n) {
    return {
      ...
      amount: BigNumber.from(_value), // ✅ OK: bigint → BigNumber
    };
  }
...
export async function findMessageFromTokenBridge(
  ...
  const iface = new Interface(messageServiceContract.contract.interface.fragments);
