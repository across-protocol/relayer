...
export async function binanceL1ToL2Finalizer(
  ...
  const [binanceDeposits, accountCoins] = await Promise.all([
    getBinanceDepositsByNetwork(binanceApi, hubSigner.provider, "ETH", fromTimestamp),
    await getAccountCoins(binanceApi),
  ]);

  logger.debug({
    at: "BinanceL1ToL2Finalizer",
    message: `Found ${binanceDeposits.length} historical L1 deposits with status === 1 on Ethereum.`,
  ...
  await mapAsync(senderAddresses, async (address) => {
    ...
    await mapAsync(SUPPORTED_TOKENS[chainId], async (_symbol) => {
      ...
      const withdrawals = await getBinanceWithdrawalsByNetwork(binanceApi, "BSC", symbol, fromTimestamp);
      ...
      if (amountToFinalize >= Number(networkLimits.withdrawMin)) {
        ...
        logger.info({
          at: "BinanceL1ToL2Finalizer",
          message: `Finalized deposit on BSC for ${amountToFinalize} ${symbol}`,
          amount: amountToFinalize,
          withdrawalId,
        });
      } else {
