...
/**
 *
 * @notice This function ensures that there's a match between ZK API.vkey AND SP1Helios.vkey. If there's not a match,
 * then the ZK proof generated by the API won't work for the contract. To avoid generating and storing a proof with an
 * incorrect ELF, this function throws early until the mismatch is fixed. When upgrading the ZK Helios setup, ZK API
 * should be updated first with a new ELF so that this check catches all the requests to ZK generation early and we don't
 * end up storing proofs with an incorrect(old) ELF. Once the upgrade on the contracts side goes through, this finalizer
