...
export class OFTBridge extends BaseBridgeAdapter<Signer, Signer | Provider> {
  ...
  constructor(
    ...
    assert(
      ...
      `OFT bridge only supports Ethereum as hub chain, got chain ID: ${srcChainId}`
    );

    // Route discovery via configured IOFT messengers: if both L1 and L2 messengers exist, the route exists
    const l1OftMessenger = OFT.getMessengerEvm(l1TokenAddress, srcChainId);
    const l2OftMessenger = OFT.getMessengerEvm(l1TokenAddress, dstChainId);

    super(dstChainId, srcChainId, srcSigner, dstSignerOrProvider, [l1OftMessenger]);

    this.l2TokenAddress = this.resolveL2TokenAddress(l1TokenAddress);
    this.l1ChainEid = OFT.getEndpointId(srcChainId);
    this.l2ChainEid = OFT.getEndpointId(dstChainId);
    this.l1Bridge = new Contract(l1OftMessenger.toNative(), IOFT_ABI_FULL, srcSigner);
    this.l2Bridge = new Contract(l2OftMessenger.toNative(), IOFT_ABI_FULL, dstSignerOrProvider);
    this.nativeFeeCap = OFT_FEE_CAP_OVERRIDES[this.srcChainId] ?? OFT_DEFAULT_FEE_CAP;
    this.l1TokenInfo = getTokenInfo(this.l1TokenAddress, this.srcChainId);
  }
