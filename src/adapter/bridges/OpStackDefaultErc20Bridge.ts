...
export class OpStackDefaultERC20Bridge extends BaseBridgeAdapter {
  ...
  async constructL1ToL2Txn(
    ...
    return Promise.resolve({
      ...
      method: "depositERC20",
      args: [l1Token.toEvmAddress(), l2Token.toEvmAddress(), amount, this.l2Gas, "0x"],
    });
  }
  ...
  async queryL1BridgeInitiationEvents(
    ...
    const events = await paginatedEventQuery(
      this.getL1Bridge(),
      this.getL1Bridge().filters.ERC20DepositInitiated(l1Token.toEvmAddress(), undefined, fromAddress.toEvmAddress()),
      eventConfig
    );
    return {
  ...
  async queryL2BridgeFinalizationEvents(
    ...
    const events = await paginatedEventQuery(
      this.getL2Bridge(),
      this.getL2Bridge().filters.DepositFinalized(l1Token.toEvmAddress(), undefined, fromAddress.toEvmAddress()),
      eventConfig
    );
    return {
      [this.resolveL2TokenAddress(l1Token)]: events.map((event) => processEvent(event, "_amount")),
