 ...
 * through the L2BaseToken system contract. The funds arrive on L1 as whatever the chain registered as its base
 * token (`Bridgehub.baseToken(chainId)`):
 *   - zkSync Era's base token is ETH, delivered as a plain native transfer and re-wrapped on the hub chain
 *     (`AdapterManager.chainsToWrapEtherOn` includes MAINNET);
 *   - Lens registered LGHO — the wrapper itself, and the L1 "WGHO" the inventory book is keyed on — so the
 *     withdrawal is minted by the L1 native token vault as exactly that ERC20 and needs no re-wrap.
 */
