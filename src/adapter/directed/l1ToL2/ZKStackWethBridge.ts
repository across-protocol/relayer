...
export class ZKStackWethBridge extends ZKStackBridge {
  ...
  override async constructL1ToL2Txn(
    ...
    return {
      ...
      method: "bridgeWeth",
      args: [this.dstChainId, netValue, amount, feeAmount, bridgeCalldata],
    };
  }
  ...
  override async queryL1BridgeInitiationEvents(
    ...
    if (isL2Contract) {
      ...
      const events = await paginatedEventQuery(
        ...
        this.getAtomicDepositor().filters.AtomicWethDepositInitiated(fromAddress.toNative(), this.dstChainId),
        eventConfig
      );
