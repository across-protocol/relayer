...
export class ZKStackBridge extends BaseBridgeAdapter<Signer, Signer | Provider> {
  ...
  async queryL1BridgeInitiationEvents(
    ...
    if (!bridgingCustomGasToken) {
      ...
      const rawEvents = await paginatedEventQuery(
        ...
        this.nativeTokenVault.filters.BridgeBurn(this.dstChainId, assetId, annotatedFromAddress),
        eventConfig
      );
      ...
      if (isL2Contract) {
        ...
        const rawEvents = await paginatedEventQuery(
          this.sharedBridge,
          this.sharedBridge.filters.BridgehubDepositBaseTokenInitiated(this.dstChainId, annotatedFromAddress),
          eventConfig
        );
  ...
  async queryL2BridgeFinalizationEvents(
    ...
    if (!bridgingCustomGasToken) {
      ...
      const events = await paginatedEventQuery(
        this.getL2Bridge(),
        this.getL2Bridge().filters.BridgeMint(this.srcChainId, assetId),
        eventConfig
      );
  ...
  protected override resolveL2TokenAddress(l1Token: EvmAddress): string {
    ...
    if (
      compareAddressesSimple(TOKEN_SYMBOLS_MAP.USDC.addresses[this.srcChainId], l1Token.toNative()) &&
      isDefined(TOKEN_SYMBOLS_MAP.USDC.addresses[this.dstChainId])
    ) {
