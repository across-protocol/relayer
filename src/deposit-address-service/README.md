...
the execute endpoint rejects as below the minimum becomes a refund withdraw. Every v3 classification is
actionable — the handler diverts anything that is not a `correct_transfer` to the refund path. The honest
home for the word is `BroadcastPendingState.operation`, which records what the transaction being broadcast
actually does, at the point it is known.
...
`executeWithdraw` serves a `mis_route` or `intent_refund` — an expired intent refunds to the deposit
address itself, so it needs the same second hop out to the committed refund address — and a
`correct_transfer` the execute endpoint rejected as `AMOUNT_BELOW_MINIMUM`. That rejection is terminal at
the API, so the fallback runs immediately, **under the same lock**, as the polling bot holds its in-flight
lock across `initiateWithdrawV3`. No `refund_only` marker is recorded: a redelivery re-calls `/execute`,
gets the same rejection, and falls through again.
