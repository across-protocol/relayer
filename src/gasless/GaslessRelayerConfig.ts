...
export class GaslessRelayerConfig extends CommonConfig {
   ...
   * Addresses stored lowercase. From `RELAYER_GASLESS_BLOCKED_ADDRESSES` (JSON string array).
   */
  ...
  constructor(env: ProcessEnv) {
    ...
    if (RELAYER_GASLESS_BLOCKED_ADDRESSES.trim().length > 0) {
      this.blockedAddresses = new Set(
        parseJson.stringArray(RELAYER_GASLESS_BLOCKED_ADDRESSES).map((address) => {
          try {
            ...
            throw new Error(`Invalid address in RELAYER_GASLESS_BLOCKED_ADDRESSES: "${address}"`);
          }
        })
      );
    }
