...
export type RelayerRefundRow = {
  chainId: number;
  chain: string;
  upcomingRefunds: BigNumber;
};

export type RelayerTokenReport = {
  l1Token: L1Token;
  tokenTotal: BigNumber;
  currentTotal: BigNumber;
  pendingTotal: BigNumber;
  upcomingRefundsTotal: BigNumber;
  rows: RelayerBalanceRow[];
  refundRows: RelayerRefundRow[];
};

export type RelayerBalanceReport = {
  [relayerAddress: string]: {
    [l1TokenSymbol: string]: RelayerTokenReport;
  };
};
...
export class Monitor {
  ...
  async computeRelayerBalances(): Promise<RelayerBalanceReport> {
    ...
    for (const relayer of relayers) {
      const relayerKey = relayer.toNative();
      report[relayerKey] = {};
      ...
      const [allRawBalances, pendingL2Withdrawals] = await Promise.all([
        ...
        (async () => {
          ...
          this.logger.debug({
            at: "Monitor#computeRelayerBalances",
      ...
      for (const l1Token of allL1Tokens) {
        ...
        const rows: RelayerBalanceRow[] = [];
        const refundRows: RelayerRefundRow[] = [];
        let currentTotal = bnZero;
        let pendingTotal = bnZero;
        let upcomingRefundsTotal = bnZero;
        ...
        for (const chainId of this.monitorChains) {
          ...
          for (const l2Token of l2Tokens) {
            ...
            currentTotal = currentTotal.add(currentBalance);
          ...
          if (upcomingRefunds.gt(0)) {
            upcomingRefundsTotal = upcomingRefundsTotal.add(upcomingRefunds);
            refundRows.push({
              chainId,
              chain: getNetworkName(chainId),
              upcomingRefunds,
        ...
        const tokenTotal = currentTotal.add(pendingTotal).add(upcomingRefundsTotal);
        report[relayerKey][l1Token.symbol] = {
          l1Token,
          tokenTotal,
          currentTotal,
          pendingTotal,
          upcomingRefundsTotal,
          rows,
          refundRows,
        };
      }
  ...
  async reportRelayerBalances(): Promise<void> {
    const report = await this.computeRelayerBalances();

    for (const [relayerAddress, perToken] of Object.entries(report)) {
      for (const [l1TokenSymbol, tokenReport] of Object.entries(perToken)) {
        if (tokenReport.tokenTotal.lte(0)) {
          continue;
        }

        const formatWei = createFormatFunction(2, 4, false, tokenReport.l1Token.decimals);
        const formattedRows = tokenReport.rows.map((row) => ({
          chain: row.chain,
          token: row.tokenSymbol,
          current: formatWei(row.current.toString()),
          pending: formatWei(row.pending.toString()),
          total: formatWei(row.total.toString()),
        }));
        const formattedRefundRows = tokenReport.refundRows.map((row) => ({
          chain: row.chain,
          token: "refunds",
          current: "-",
          pending: "-",
          total: formatWei(row.upcomingRefunds.toString()),
        }));
        const allFormattedRows = [...formattedRows, ...formattedRefundRows];

        const totalFormatted = formatWei(tokenReport.tokenTotal.toString());
        const valueWidth = Math.max(
          ...allFormattedRows.flatMap((r) => [r.current, r.pending, r.total].map((v) => v.length)),
          totalFormatted.length
        );
        let tokenMrkdwn = "```\n";
        for (const row of allFormattedRows) {
        ...
        this.logger.info({
          ...
          message: `Balance report for ${relayerAddress} [${l1TokenSymbol}]`,
          mrkdwn: tokenMrkdwn,
        });
      }
    }
