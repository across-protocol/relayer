...
Swap deposits into Binance are tagged `SWAP` in Redis with infinite TTL so the finalizer excludes them 
from its deposit ledger for the whole lookback — including after the swap completes, so a consumed deposit is never 
re-counted as finalizable ("phantom" `amountToFinalize`). Handover to the finalizer is driven by order state, not
