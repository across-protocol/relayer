...
export async function run(): Promise<void> {
  ...
  console.log(
    `Current WETH balance for account ${signerAddr}: ${fromWei(
      currentBalance,
      18 /* l1TokenInfo.decimals*/
    )} ${l2Token}`
  );
  console.log(
    `Current ETH balance for account ${signerAddr}: ${fromWei(currentEthBalance, 18 /* l1TokenInfo.decimals*/)}`
  );
