...
// Example run:
// ts-node ./scripts/withdrawUsdtFromL2.ts --chainId 143 --amount 3  # Shows calldata, doesn't execute
// ts-node ./scripts/withdrawUsdtFromL2.ts --chainId 8453 --amount 5 --sendTx --wallet gckms --keys bot1  # Actually sends transaction

const MAINNET_CHAIN_ID = CHAIN_IDs.MAINNET;
const USDT_L1_ADDRESS = TOKEN_SYMBOLS_MAP.USDT.addresses[MAINNET_CHAIN_ID];
...
async function run(): Promise<void> {
  ...
  if (!args.amount) {
    throw new Error("Define `amount` as the amount of USDT to withdraw (e.g., --amount 3 for 3 USDT)");
  }
  ...
  console.log(`🚀 Withdrawing USDT from ${l2ChainName} (Chain ID: ${l2ChainId}) to Mainnet via OFT`);
  ...
  // Get token addresses
  const l1Token = EvmAddress.from(USDT_L1_ADDRESS);
  const l2Token = getRemoteTokenForL1Token(l1Token, l2ChainId, MAINNET_CHAIN_ID);
  if (!isDefined(l2Token)) {
    throw new Error(`USDT not found on ${l2ChainName} (chain ID: ${l2ChainId})`);
  }
  ...
  // Check balance on L2
  const usdtContract = new Contract(l2Token.toNative(), ERC20.abi, l2Signer);
  const balance = await usdtContract.balanceOf(signerAddr);
  const balanceFormatted = formatter(balance.toString());

  console.log(`\n📊 Current USDT balance on ${l2ChainName}: ${balanceFormatted} USDT`);
  console.log(`💸 Amount to withdraw: ${formatter(amountInWei.toString())} USDT`);

  if (balance.lt(amountInWei)) {
    throw new Error(
      `Insufficient balance! You have ${balanceFormatted} USDT on ${l2ChainName}, but trying to withdraw ${formatter(
        amountInWei.toString()
      )} USDT`
    );
  }

  // Determine which L2 bridge to use (should be OFTL2Bridge for USDT)
  ...
  if (!isDefined(BridgeConstructor)) {
    throw new Error(
      `No L2 bridge configured for USDT on ${l2ChainName} (chain ID: ${l2ChainId}). Check CUSTOM_L2_BRIDGE and CANONICAL_L2_BRIDGE in Constants.ts`
    );
  }
  ...
  console.log(`   Amount: ${formatter(amountInWei.toString())} USDT`);
  ...
  if (!sendTransactions) {
    ...
    console.log(
      `   Example: yarn ts-node ./scripts/withdrawUsdtFromL2.ts --chainId ${l2ChainId} --amount ${withdrawAmount} --sendTx --wallet gckms --keys bot1`
    );
    return;
  }
  ...
  const txnReceipts = await multicallerClient.executeTxnQueues(false, [l2ChainId]); // simMode is false here to actually send
  ...
  console.log("   You can monitor the finalization on Mainnet once the OFT bridge message is processed.");
}
