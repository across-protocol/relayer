...
import { getDeployedContract, getProvider, CHAIN_IDs, resolveTokenBySymbol } from "../src/utils";
...
export function resolveToken(token: string, chainId: number): ERC20 {
  ...
  const _token = symbol !== undefined ? resolveTokenBySymbol(symbol) : undefined;
  if (_token === undefined) {
