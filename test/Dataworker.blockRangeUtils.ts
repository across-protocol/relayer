...
describe("Dataworker block range-related utility methods", async function () {
  ...
  it("DataworkerUtils.blockRangesAreInvalidForSpokeClients", async function () {
    ...
    mockSpokePoolClient.setBlockTimestamp(mockSpokePoolClient.eventSearchConfig.fromBlock,  endBlockTimestamps[originChainId] - 1);
    const expectedTimeBetweenOldestAndEndBlockTimestamp =
      endBlockTimestamps[originChainId] - await mockSpokePoolClient.getTimeAt(mockSpokePoolClient.eventSearchConfig.fromBlock);
    assert(
    ...
    mockSpokePoolClient.setBlockTimestamp(mockSpokePoolClient.eventSearchConfig.fromBlock, oldestBlockTimestampOverride);
    result = await blockRangesAreInvalidForSpokeClients(
      { [originChainId]: mockSpokePoolClient as SpokePoolClient },
      blockRanges,
      chainIds,
