...
describe("InventoryClient: Rebalancing inventory", async function () {
  ...
  it("Correctly decides when to execute rebalances: token shortfall", async function () {
    ...
    const shortfallAmount1 = toWei(10);
    const shortfallAmount2 = toWei(8);
    tokenClient.setTokenShortFallData(
      POLYGON,
      toAddressType(l2TokensForWeth[POLYGON], POLYGON),
      [BigNumber.from(6969), BigNumber.from(6970)],
      [shortfallAmount1, shortfallAmount2]
    );
    await inventoryClient.update();

    // There should be a rebalance sent for each shortfall.
    ...
    const expectedBridgedAmount = toWei(10).add(toWei(8));
    ...
    // Now, mock funds having entered the canonical bridge. If we only mock enough to cover one shortfall,
    // then we should see a rebalance for the other.
    adapterManager.setMockedOutstandingCrossChainTransfers(
      POLYGON,
      EvmAddress.from(owner.address),
      EvmAddress.from(mainnetWeth),
    ...
    mainnetWethContract.balanceOf
      ...
      .returns(initialAllocation[MAINNET][mainnetWeth].sub(expectedBridgedAmount));

    // There should be one rebalance:
    await inventoryClient.update();
    await inventoryClient.rebalanceInventoryIfNeeded();
    expect(lastSpyLogIncludes(spy, "Executed Inventory rebalances")).to.be.true;
    expect(lastSpyLogIncludes(spy, "Rebalances sent to Polygon")).to.be.true;
    ...
    // Now, mock enough funds to cover the total shortfall.
    adapterManager.setMockedOutstandingCrossChainTransfers(
      POLYGON,
      EvmAddress.from(owner.address),
      EvmAddress.from(mainnetWeth),
  ...
  describe("Tempo inventory-equivalent contributors", function () {
    ...
    const buildTempoInventoryClient = (useAliasConfig = false) => {
      const tempoInventoryConfig: InventoryConfig = {
        ...
        tokenConfig: {
          [mainnetUsdc]: useAliasConfig
            ? {
                [tempoUsdc]: {
                  [TEMPO]: { targetPct: toWei(0.1), thresholdPct: toWei(0.05), targetOverageBuffer },
                },
                [tempoPathUsd]: {
            ...
            : {
                [TEMPO]: { targetPct: toWei(0.3), thresholdPct: toWei(0.2), targetOverageBuffer },
              },
        },
      };
      ...
      tempoTokenClient.setTokenData(TEMPO, toAddressType(tempoUsdc, TEMPO), toMegaWei(100));
      tempoTokenClient.setTokenData(TEMPO, toAddressType(tempoPathUsd, TEMPO), toMegaWei(200));

      return { tempoInventoryClient };
    };

    it("Counts Tempo pathUSD in aggregate USDC balances while keeping canonical routing on USDC.e", function () {
      const { tempoInventoryClient } = buildTempoInventoryClient();
      ...
      expect(tempoInventoryClient.getBalanceOnChain(TEMPO, EvmAddress.from(mainnetUsdc)).eq(toMegaWei(300))).to.be.true;
      expect(
        tempoInventoryClient
          .getBalanceOnChain(TEMPO, EvmAddress.from(mainnetUsdc), toAddressType(tempoUsdc, TEMPO))
          .eq(toMegaWei(100))
      ).to.be.true;
      expect(
        tempoInventoryClient
          .getBalanceOnChain(TEMPO, EvmAddress.from(mainnetUsdc), toAddressType(tempoPathUsd, TEMPO))
          .eq(toMegaWei(200))
      ).to.be.true;
      expect(tempoInventoryClient.getCumulativeBalance(EvmAddress.from(mainnetUsdc)).eq(toMegaWei(1300))).to.be.true;
      expect(
        tempoInventoryClient
          .getCurrentAllocationPct(EvmAddress.from(mainnetUsdc), TEMPO, toAddressType(tempoUsdc, TEMPO))
          .eq(toWei("3").div(13))
      ).to.be.true;

      const distribution = tempoInventoryClient.getTokenDistributionPerL1Token();
      expect(distribution[mainnetUsdc][TEMPO][tempoUsdc].eq(toWei("3").div(13))).to.be.true;
    });

    it("Preserves per-token allocation accounting when Tempo contributors are configured via alias config", function () {
      const { tempoInventoryClient } = buildTempoInventoryClient(true);
      ...
      expect(
        tempoInventoryClient
          .getCurrentAllocationPct(EvmAddress.from(mainnetUsdc), TEMPO, toAddressType(tempoPathUsd, TEMPO))
          .eq(toWei("2").div(13))
      ).to.be.true;
    });
  });
