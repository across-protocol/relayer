...
describe("EventManager: removeAbove", async function () {
  const chainId = CHAIN_IDs.MAINNET;
  ...
  const makeEvent = (blockNumber: number): Log => ({
    ...
    removed: false,
    address: randomAddress(),
    data: ethersUtils.id(`removeAbove-random-${randomNumber()}`),
    topics: [],
    args: {},
    blockHash: makeHash(),
    event: "testEvent",
  ...
  it("Removes events above fork point", async function () {
    const event100 = makeEvent(100);
    const event105 = makeEvent(105);
    const event110 = makeEvent(110);

    eventMgr.add(event100, providers[0]);
    eventMgr.add(event105, providers[0]);
    eventMgr.add(event110, providers[0]);

    eventMgr.removeAbove(105);

    // Block 110 event should be gone; 100 and 105 should remain.
    const hash100 = eventMgr.hashEvent(event100);
    const hash105 = eventMgr.hashEvent(event105);
    const hash110 = eventMgr.hashEvent(event110);

    expect(eventMgr.findEvent(hash100)).to.exist;
    expect(eventMgr.findEvent(hash105)).to.exist;
    expect(eventMgr.findEvent(hash110)).to.not.exist;
  });

  it("Returns removed events", async function () {
    const event100 = makeEvent(100);
    const event110 = makeEvent(110);
    ...
    const removed = eventMgr.removeAbove(105);

    expect(removed).to.have.lengthOf(1);
    expect(removed[0].blockNumber).to.equal(110);
    expect(removed[0].transactionHash).to.equal(event110.transactionHash);
  });

  it("Cleans up internal maps", async function () {
    const event100 = makeEvent(100);
    const event110 = makeEvent(110);
    ...
    // blockNumbers and blockHashes for the removed block should be gone.
    expect(eventMgr.blockNumbers.has(110)).to.be.false;
    expect(eventMgr.blockHashes[event110.blockHash]).to.not.exist;

    // Retained block's entries should remain.
    expect(eventMgr.blockNumbers.has(100)).to.be.true;
    expect(eventMgr.blockHashes[event100.blockHash]).to.exist;
  });

  it("No-op when nothing above fork point", async function () {
    const event100 = makeEvent(100);
    const event105 = makeEvent(105);
    ...
    const removed = eventMgr.removeAbove(110);
    ...
    expect(eventMgr.findEvent(eventMgr.hashEvent(event105))).to.exist;
  });

  it("Handles multiple events at same block with same hash", async function () {
    const event110a = makeEvent(110);
    const event110b = { ...makeEvent(110), blockHash: event110a.blockHash };

    eventMgr.add(event110a, providers[0]);
    eventMgr.add(event110b, providers[0]);

    const removed = eventMgr.removeAbove(105);
    expect(removed).to.have.lengthOf(2);

    expect(eventMgr.findEvent(eventMgr.hashEvent(event110a))).to.not.exist;
    expect(eventMgr.findEvent(eventMgr.hashEvent(event110b))).to.not.exist;
    expect(eventMgr.blockNumbers.has(110)).to.be.false;
    expect(eventMgr.blockHashes[event110a.blockHash]).to.not.exist;
  });

  it("Handles events with different block hashes at same block number", async function () {
    const event110a = makeEvent(110);
    const event110b = makeEvent(110); // Different blockHash from event110a.
    ...
    expect(eventMgr.blockNumbers.has(110)).to.be.false;
    expect(eventMgr.blockHashes[event110a.blockHash]).to.not.exist;
    expect(eventMgr.blockHashes[event110b.blockHash]).to.not.exist;
  });
});
