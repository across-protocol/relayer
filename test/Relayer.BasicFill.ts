...
describe("Relayer: Check for Unfilled Deposits and Fill", async function () {
  ...
  describe("Relayer: Check for Unfilled v3 Deposits and Fill", async function () {
    ...
    describe.only("Auxiliary native cost", function () {
      const configureProfitScenario = ({
        ...
        // Neutralize padding/multiplier for deterministic assertions
        profitClient.setGasPadding(toBNWei("1"));
        profitClient.setGasMultiplier(toBNWei("1"));
      };

      const setUnitPrices = async (): Promise<void> => {
        ...
        profitClient.setTokenPrice(nativeToken.symbol, toBNWei("1"));
        profitClient.setTokenPrice(gasToken.symbol, toBNWei("1"));
        // Also ensure L1 token used for fee math is 1 USD
        profitClient.setTokenPrice(await l1Token.symbol(), toBNWei("1"));
      };

      it("fills deposits when auxiliary cost stays within margin", async function () {
        await setUnitPrices();
        configureProfitScenario({});

        // Construct a deposit with a non-empty message carrying a small value_amount on SVM model.
        const smallAuxMessage = "0x01"; // Non-empty placeholder; arch.svm.deserializeMessage will be bypassed on EVM
      ...
      it("skips filling when auxiliary cost pushes net fee below min", async function () {
        await setUnitPrices();
        configureProfitScenario({});

        // Simulate a large auxiliary value by embedding a longer non-empty message.
        const largeAuxMessage = "0x0123"; // Non-empty placeholder
      ...
      it("handles absurd auxiliary cost larger than outputAmount without anomalies", async function () {
        await setUnitPrices();
        configureProfitScenario({});

        // Use a clearly non-empty message; in production SVM, value_amount would come from message bytes.
        const absurdAuxMessage = "0xdeadbeef";
        ...
        const deposit = await depositV3(
          ...
          erc20_2.address,
          toBNWei("100"),
          { message: absurdAuxMessage }
        );
        await updateAllClients();

        const fill = await profitClient.calculateFillProfitability(deposit, bnZero, bnZero);
        expect(fill.auxiliaryNativeTokenCost.gte(bnZero)).to.be.true; // defined and non-negative
        expect(fill.nativeTokenFillCostUsd.gte(fill.gasCostUsd)).to.be.true; // includes aux
        expect(fill.profitable).to.be.false; // should be unprofitable
