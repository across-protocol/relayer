...
- pending-state reads never throw out of the client but are all-or-nothing: if any adapter's `getPendingRebalances` throws, the whole aggregate degrades to empty (logged with a warning, reported via `getPendingReadFailures()`) rather than failing the read or returning partial data. Partial aggregates are unsafe because exchange adapters (Binance/Hyperliquid) emit debits that cancel the bridge adapters' (CCTP/OFT) credits for the same in-flight order, so dropping only the failed adapter could overcount balances on the bridge destination chain. When the aggregate is degraded, the rebalancer runtime skips sending new rebalances for that run (fail closed), while read-only consumers (relayer `InventoryClient`, monitor) keep running with in-flight rebalances temporarily uncounted.
